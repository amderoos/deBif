...
phaseplane <- function(model, state, parms) {
  ...
  ui <- dashboardPagePlus(
    ...
    sidebar = dashboardSidebar(width = 220,
      sidebarMenu(
        menuItem(h4("Initial state"),
          lapply(1:length(state), function(i) {menuItem(names(state[i]), numericInput(inputId=names(state[i]), label=NULL, value=as.numeric(state[i]), step=0.1*as.numeric(state[i]), width="95%"), startExpanded = TRUE)})
        ),
        menuItem(h4("Parameters"),
          lapply(1:length(parms), function(i) {menuItem(names(parms[i]), numericInput(inputId=names(parms[i]), label=NULL, value=as.numeric(parms[i]), step=0.1*as.numeric(parms[i]), width="95%"), startExpanded = TRUE)})
        ),
        menuItem(actionButton("lapply", "Apply", icon("refresh")))
      )
    ),
    body = dashboardBody(
      do.call(tabsetPanel, c(myTabs, id = "plottab"))
    #  , box(width = NULL, height = NULL, verbatimTextOutput("debug"))
    ),
    rightsidebar = rightSidebar(
      ...
      rightSidebarTabContent(
        ...
        icon = "chart-line",
        active = TRUE,
        numericInput(inputId="xmin", label="Minimum x-axis", value=0),
        numericInput(inputId="xmax", label="Maximum x-axis", value=100),
        selectInput('xscale', 'Scale x-axis', c('Linear', "Logarithmic")),
        numericInput(inputId="ymin", label="Minimum y-axis", value=0),
        numericInput(inputId="ymax", label="Maximum y-axis", value=1),
        selectInput('yscale', 'Scale y-axes', c('Linear', "Logarithmic"))
      ),
      rightSidebarTabContent(
        id = "numopttab",
      ...
      br(),
      actionButton("rapply", "Apply", icon("refresh"))
    )
  )
      ...
      output[[paste0("plot", nr)]] <- renderPlot({
        ...
        isolate({
          ...
          if (plottype == 0) {
            ...
            lapply(2:ncol(nsol), function(i) {lines(nsol[,1], nsol[,i], col=debifOpts$colors[min(i-1, length(debifOpts$colors))], lwd=debifOpts$plotopts["lwd"], pch=(i-1))})
            ...
            if (length(state) == 1) {
              ...
              allequi(state=state, parms=parms, odes=model, x=1, xmin=xmin, xmax=xmax, log=logxy, grid=10, plot=TRUE)
            }
            else {
              plane2D(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, log=logxy, odes=model, state=state, parms=parms, eps=-.001, dbopts=debifOpts)
              allequi(state=state, parms=parms, odes=model, x=1, xmin=xmin, xmax=xmax, y=2, ymin=ymin, ymax=ymax, log=logxy, grid=10, plot=TRUE)
            }
          }
          else if (plottype == 3) {
            plane2D(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, log=logxy, tmax=tmax, tstep=tstep, odes=model, state=state, parms=parms, eps=-.001, vector=TRUE, grid=8, dbopts=debifOpts)
            ...
            plane2D(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, log=logxy, tmax=tmax, tstep=tstep, odes=model, state=state, parms=parms, eps=-.001, portrait=TRUE, dbopts=debifOpts, method=input$method)
            allequi(state=state, parms=parms, odes=model, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, log=logxy, grid=10, report=FALSE, eigenvalues=FALSE, plot=TRUE)
          }
        })
