...
#'
#'
#'   bifurcation(model, state, parms, inlist = NULL, silent = FALSE)
#'
#'
...
#' }
#' @importFrom graphics contour legend lines par plot points text axis mtext
...
#' @export
bifurcation <- function(model, state, parms, inlist = NULL, silent = FALSE) {
  ...
  for (i in 1:length(plotopts)) {
    plotopts[[i]] <- list(xcol = 1, xmin = 0, xmax = 1, logx = 0, xlab = "",
                          ...
                          y2col = 1, y2min = 0, y2max = 1, logy2 = 0, y2lab = "None",
  ...
  plotopts[[1]]$xlab <- "Time"
  plotopts[[1]]$ylab <- "State variables"
  plotopts[[2]]$xlab <- names(parms)[1]
  plotopts[[2]]$ylab <- "State variables"
  plotopts[[3]]$xlab <- names(parms)[1]
  plotopts[[3]]$ycol <- 2
  plotopts[[3]]$ylab <- names(parms)[2]
  ...
  numopts <- list(odemethod = "lsoda", tmax = 100, tstep = 0.1,
                  ...
                  maxpoints = 500, iszero = 1.0E-5, stepsize = 0.01, minstepsize = 1.0E-5, computedelay = 0.0
  )
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$appendbtn, {
      processLoadCurve(input, serverenv$output, session, bifenv, replace = FALSE)
    ...
    observeEvent(input$replacebtn, {
      processLoadCurve(input, serverenv$output, session, bifenv, replace = TRUE)
